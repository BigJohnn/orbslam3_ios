...
void KeyFrameDatabase::clearMap(Map* pMap)
{
    ...
    // Erase elements in the Inverse File for the entry
    for(std::vector<list<std::shared_ptr<KeyFrame>> >::iterator vit=mvInvertedFile.begin(), vend=mvInvertedFile.end(); vit!=vend; vit++)
    {
        // List of keyframes that share the word
...
void KeyFrameDatabase::DetectCandidates(std::shared_ptr<KeyFrame> pKF, float minScore,vector<std::shared_ptr<KeyFrame>>& vpLoopCand, vector<std::shared_ptr<KeyFrame>>& vpMergeCand)
{
    set<std::shared_ptr<KeyFrame>> spConnectedKeyFrames = pKF->GetConnectedKeyFrames();
    ...
    if(!lKFsSharingWordsLoop.empty())
    {
        ...
        // Only compare against those keyframes that share enough words
        int maxCommonWords=0;
        for(auto lit=lKFsSharingWordsLoop.begin(), lend= lKFsSharingWordsLoop.end(); lit!=lend; lit++)
        {
            if((*lit)->mnLoopWords>maxCommonWords)
                maxCommonWords=(*lit)->mnLoopWords;
        }
    ...
    if(!lKFsSharingWordsMerge.empty())
    {
        ...
        // Only compare against those keyframes that share enough words
        int maxCommonWords=0;
        for(auto lit=lKFsSharingWordsMerge.begin(), lend=lKFsSharingWordsMerge.end(); lit!=lend; lit++)
        {
...
void KeyFrameDatabase::DetectBestCandidates(std::shared_ptr<KeyFrame>pKF, vector<std::shared_ptr<KeyFrame>> &vpLoopCand, vector<std::shared_ptr<KeyFrame>> &vpMergeCand, int nMinWords)
{
    list<std::shared_ptr<KeyFrame>> lKFsSharingWords;
...
bool compFirst(const pair<float, std::shared_ptr<KeyFrame>> & a, const pair<float, std::shared_ptr<KeyFrame>> & b)
{
    return a.first > b.first;
}


void KeyFrameDatabase::DetectNBestCandidates(std::shared_ptr<KeyFrame>pKF, vector<std::shared_ptr<KeyFrame>> &vpLoopCand, vector<std::shared_ptr<KeyFrame>> &vpMergeCand, int nNumCandidates)
{
    list<std::shared_ptr<KeyFrame>> lKFsSharingWords;
    set<std::shared_ptr<KeyFrame>> spConnectedKF;
