...
void Map::AddKeyFrame(std::shared_ptr<KeyFrame> const& pKF)
{
...
void Map::AddMapPoint(std::shared_ptr<MapPoint> const& pMP)
{
    unique_lock<mutex> lock(mMutexMap);
...
void Map::EraseMapPoint(shared_ptr<MapPoint> const& pMP)
{
    unique_lock<mutex> lock(mMutexMap);
...
void Map::EraseKeyFrame(std::shared_ptr<KeyFrame> const& pKF)
{
    unique_lock<mutex> lock(mMutexMap);
    ...
    if(mspKeyFrames.size()>0)
    {
        if(pKF->mnId == mpKFlowerID->mnId)
        {
            auto&& vpKFs = vector<shared_ptr<KeyFrame>>(mspKeyFrames.begin(),mspKeyFrames.end());
