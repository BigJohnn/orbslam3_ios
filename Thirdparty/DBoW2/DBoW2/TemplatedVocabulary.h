...
public:
  ...
  void setScoringType(ScoringType type);
     ...
     * @param filename
     */
  bool loadFromBinFile(const std::string &filename, cv::Mat* buf2del_by_user);
...
#include <opencv2/core/core.hpp>
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor,F>::loadFromBinFile(const std::string &filename, cv::Mat* buf2del_by_user)
{
    // Open file in binary mode
      ...
      // ... (rest of the code using the buffer can remain similar)
    const char* ptr = buffer;

      unsigned int nb_nodes, size_node;
        nb_nodes = *(unsigned int*)ptr;
        ptr += sizeof(unsigned int);
        size_node = *(unsigned int*)ptr;
        ptr += sizeof(unsigned int);
        m_k = *(int*)ptr;
        ptr += sizeof(int);
        m_L = *(int*)ptr;
        ptr += sizeof(int);
        m_scoring = *(ScoringType*)ptr;
        ptr += sizeof(ScoringType);
        m_weighting = *(WeightingType*)ptr;
        ptr += sizeof(WeightingType);
  //      printf("nb_nodes = %d, size_nodes = %d, m_k = %d, m_L = %d, m_scoreing = %d, m_weighting = %d\n",
  //           nb_nodes, size_node, m_k, m_L, m_scoring, m_weighting);
      createScoringObject();
      ...
      buf2del_by_user = new cv::Mat(1, F::L * nb_nodes, CV_8U);
  ...
  //        const int* ptr=(int*)buf;
          m_nodes[nid].parent = *buf;
          //m_nodes[nid].parent = *(const int*)buf;
          m_nodes[m_nodes[nid].parent].children.push_back(nid);

          m_nodes[nid].descriptor = cv::Mat(std::vector<int>(1, F::L), CV_8U, buf2del_by_user->data + sizeof(unsigned char*) * (nid - 1));
          memcpy(m_nodes[nid].descriptor.data, ptr+4, F::L);

          m_nodes[nid].weight = *(float*)(ptr+4+F::L);
    ...
    //TODO: Check!
//        printf("m_nodes[0]=> %lu", m_nodes[0].children.size());
//        printf("m_nodes[1]=> %lu", m_nodes[1].children.size());
//        printf("m_nodes[2]=> %lu", m_nodes[2].children.size());
//        printf("m_nodes[2].weight=> %lu", m_nodes[2].weight);
//        printf("m_nodes[2].parent=> %lu", m_nodes[2].parent);
//        printf("m_nodes[42]=> %lu", m_nodes[42].children.size());
//        printf("m_nodes[82]=> %lu", m_nodes[82].children.size());
//        printf("m_nodes[428]=> %lu", m_nodes[428].children.size());
//    
//        printf("m_nodes[199].descriptor cols=> %lu", m_nodes[199].descriptor.cols);
//        printf("m_nodes[199].descriptor rows=> %lu", m_nodes[199].descriptor.rows);
//        printf("m_nodes[199].descriptor[28]=> %d", m_nodes[199].descriptor.template at<int>(0,28));
//    
//          printf("m_nodes[1].descriptor cols=> %lu", m_nodes[1].descriptor.cols);
//          printf("m_nodes[1].descriptor rows=> %lu", m_nodes[1].descriptor.rows);
//          printf("m_nodes[1].descriptor[28]=> %d", m_nodes[1].descriptor.template at<int>(0,28));
